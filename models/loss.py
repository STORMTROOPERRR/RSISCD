...
def get_loss(ss_loss, cr_loss):
    if ss_loss == 'ce':
        ...
        raise NotImplementedError(f"{ss_loss} not supported!")

    if cr_loss == 'similarity':
        criterion_sc = ChangeSimilarity()
    elif cr_loss == 'saliency':
        criterion_sc = ChangeSalience()
    else:
        raise NotImplementedError(f"{cr_loss} not supported!")

    return criterion.cuda(), criterion_sc.cuda()
