...
root = '/data/zcy/DATASET/Landsat-SCD/'
...
class Data(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.valid_label:
            if self.random_flip:
                img_A, img_B, label_A, label_B, valid_label = transform.rand_rot90_flip_SCD5(img_A, img_B, label_A,
                                                                                             label_B, valid_label)
            return F.resize(F.to_tensor(img_A), size=[512, 512]), \
                   F.resize(F.to_tensor(img_B), size=[512, 512]), \
                   F.resize(torch.from_numpy(label_A).unsqueeze(dim=0), size=[512, 512]).squeeze(dim=0), \
                   F.resize(torch.from_numpy(label_B).unsqueeze(dim=0), size=[512, 512]).squeeze(dim=0), \
                   F.resize(torch.from_numpy(valid_label).unsqueeze(dim=0), size=[512, 512]).squeeze(dim=0)
        else:
            if self.random_flip:
